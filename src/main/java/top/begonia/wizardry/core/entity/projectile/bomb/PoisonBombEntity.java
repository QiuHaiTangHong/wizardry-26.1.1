...
public class PoisonBombEntity extends BombEntity {
    ...
    protected void createParticles(Level level) {
        ...
        ParticleBuilder.create(WizardryParticles.FLASH.get()).pos(hitPos).scale(5 * blastMultiplier)
                .clr(0.2f + this.random.nextFloat() * 0.3f, 0.6f, 0.0f).spawn(this.level());

        for (int i = 0; i < 60 * blastMultiplier; i++) {

            ParticleBuilder.create(WizardryParticles.SPARKLE.get(), this.random, hitPos.x(), hitPos.y(), hitPos.z(), 2 * blastMultiplier, false).time(35)
                    .scale(2).clr(0.2f + this.random.nextFloat() * 0.3f, 0.6f, 0.0f).spawn(this.level());

            ParticleBuilder.create(WizardryParticles.DARK_MAGIC.get(), this.random, hitPos.x(), hitPos.y(), hitPos.z(), 2 * blastMultiplier, false)
                    .clr(0.2f + this.random.nextFloat() * 0.2f, 0.8f, 0.0f).spawn(this.level());
        }
